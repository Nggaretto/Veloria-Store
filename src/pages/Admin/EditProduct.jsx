...
// بيحول أي شكل قديم للمنتج (مقاس واحد، أو سعر عادي بس) لنفس شكل المقاسات S/M/L
// عشان لو منتج اتضاف زمان بشكل مختلف، صفحة التعديل متتعطلش
function normalizeProduct(data) {
  const sizes = { S: { ...emptySize }, M: { ...emptySize }, L: { ...emptySize } };
  let sizeMode = 'multiple';
  let oneSize = { price: '', stock: '' };

  if (data.hasMultipleSizes && data.sizes) {
    sizeMode = 'multiple';
    SIZES.forEach((s) => {
      if (data.sizes[s]) {
        sizes[s] = {
          active: !!data.sizes[s].active,
          price: data.sizes[s].price ?? '',
          stock: data.sizes[s].stock ?? '',
        };
      }
    });
  } else if (data.singleSize) {
    sizeMode = 'one';
    oneSize = { price: data.singleSize.price ?? '', stock: data.singleSize.stock ?? '' };
  } else if (data.price !== undefined) {
    sizeMode = 'one';
    oneSize = { price: data.price ?? '', stock: data.stock ?? 0 };
  }
  ...
  return {
    ...
    sizes,
    oneSize,
    discountActive: !!data.discount?.active,
...
export default function EditProduct() {
  ...
  const [sizeMode, setSizeMode] = useState('multiple');
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      const normalized = normalizeProduct(snap.data());
      setName(normalized.name);
      setDescription(normalized.description);
      setImages(normalized.images);
      setSizeMode(normalized.sizeMode);
      setSizes(normalized.sizes);
      setOneSize(normalized.oneSize);
      setDiscountActive(normalized.discountActive);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (sizeMode === 'multiple') {
        ...
        payload = { hasMultipleSizes: true, sizes: sizesPayload, singleSize: null };
      } else {
        payload = {
    ...
    <div className="p-8 bg-[#F8F3EE] min-h-screen" dir="rtl">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-bold text-[#5A2D20]">تعديل المنتج</h1>
          <Link to={`/admin/products/${id}`} className="text-[#5A2D20] font-bold hover:underline">
            ← رجوع لتفاصيل المنتج
          </Link>
        </div>
